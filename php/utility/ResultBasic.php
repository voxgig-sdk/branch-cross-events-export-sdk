...
// BranchCrossEventsExport SDK utility: result_basic

class BranchCrossEventsExportResultBasic
{
    public static function call(BranchCrossEventsExportContext $ctx): ?BranchCrossEventsExportResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof BranchCrossEventsExportError) ? $result->err->msg : (string)$result->err;
