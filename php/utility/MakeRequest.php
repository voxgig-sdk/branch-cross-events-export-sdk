...
class BranchCrossEventsExportMakeRequest
{
    public static function call(BranchCrossEventsExportContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new BranchCrossEventsExportResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new BranchCrossEventsExportResponse($fetched);
        } else {
