...
// BranchCrossEventsExport SDK retry feature
...
class BranchCrossEventsExportRetryFeature extends BranchCrossEventsExportBaseFeature
{
    ...
    public function init(BranchCrossEventsExportContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BranchCrossEventsExportContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BranchCrossEventsExportContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BranchCrossEventsExportContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
