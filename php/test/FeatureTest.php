...
// BranchCrossEventsExport SDK feature test
//
...
class FtCtrl extends BranchCrossEventsExportControl
{
...
class FtClient
{
    ...
    private BranchCrossEventsExportUtility $utility;

    public function __construct(BranchCrossEventsExportUtility $utility, array $options)
    {
    ...
    public function get_utility(): BranchCrossEventsExportUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BranchCrossEventsExportContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BranchCrossEventsExportContext $rootctx;

    // True when this SDK was generated with the named feature.
    public static function hasFeature(string $name): bool
    {
        if (self::$feature_config === null) {
            $config = BranchCrossEventsExportConfig::shared_config();
            $f = $config['feature'] ?? [];
    ...
    public function __construct(
        ...
        $utility = new BranchCrossEventsExportUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (BranchCrossEventsExportContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BranchCrossEventsExportConfig::make_feature($name);
    ...
    private static function buildUrl(BranchCrossEventsExportSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BranchCrossEventsExportError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BranchCrossEventsExportResponse($fetched);
        }
    ...
    private function fail(BranchCrossEventsExportContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BranchCrossEventsExportContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
