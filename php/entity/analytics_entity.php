...
// BranchCrossEventsExport SDK Analytics entity
...
class AnalyticsEntity
{
    ...
    {
        if ($args) {
            $this->_data = BranchCrossEventsExportHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = BranchCrossEventsExportHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *
     * @param AnalyticsCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AnalyticsCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Analytics|array The created Analytics as an assoc-array at the
     *   SDK boundary; throws BranchCrossEventsExportError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = BranchCrossEventsExportHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
