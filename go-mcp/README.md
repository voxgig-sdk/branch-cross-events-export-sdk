# branch-cross-events-export-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BranchCrossEventsExport SDK as
two agent tools — `branch-cross-events-export_list` and `branch-cross-events-export_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-cross-events-export-mcp)
make build

# 2. Provide credentials via the environment
export BRANCH_CROSS_EVENTS_EXPORT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user branch-cross-events-export \
  -- /absolute/path/to/branch-cross-events-export-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./branch-cross-events-export-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// branch-cross-events-export_list: first page of records
{ "entity": "analytics" }
{ "entity": "analytics", "query": { } }

// branch-cross-events-export_load: one record by id
{ "entity": "status", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-cross-events-export-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user branch-cross-events-export \
     -- "$PWD"/dist/*/branch-cross-events-export-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `branch-cross-events-export_list` and `branch-cross-events-export_load` tools now appear
   in new sessions. Ask the agent to *"list analytics using branch-cross-events-export"*
   and it calls `branch-cross-events-export_list` with `{"entity":"analytics"}`.
...
```sh
export BRANCH_CROSS_EVENTS_EXPORT_APIKEY=sk_live_xxx            # API key
export BRANCH_CROSS_EVENTS_EXPORT_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `branch-cross-events-export_list` tool
...
### Call the `branch-cross-events-export_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `branch-cross-events-export_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `branch-cross-events-export_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BRANCH_CROSS_EVENTS_EXPORT_APIKEY` | API key sent with every request. |
| `BRANCH_CROSS_EVENTS_EXPORT_BASE` | Optional override of the API base URL. |
...
```sh
./branch-cross-events-export-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"branch-cross-events-export_load","arguments":{"entity":"status","query":{"id":1}}}}'
```
