...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BRANCH_CROSS_EVENTS_EXPORT_TEST_LIVE") == "TRUE" ||
		os.Getenv("BRANCH_CROSS_EVENTS_EXPORT_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BRANCH_CROSS_EVENTS_EXPORT_TEST_EXPLAIN"); explain != "" {
