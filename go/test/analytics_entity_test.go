...
func TestAnalyticsEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BRANCH_CROSS_EVENTS_EXPORT_TEST_ANALYTICS_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		analyticsRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "analytics"}), "analytics_ref01"))
...
func analyticsBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BRANCH_CROSS_EVENTS_EXPORT_TEST_ANALYTICS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BRANCH_CROSS_EVENTS_EXPORT_TEST_ANALYTICS_ENTID"])
	...
	if env["BRANCH_CROSS_EVENTS_EXPORT_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewBranchCrossEventsExportSDK(core.ToMapAny(mergedOpts))
	}
