...
import (
	...
	omni "github.com/voxgig-sdk/branch-cross-events-export-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BranchCrossEventsExportError); is {
				out := map[string]any{
					"name":    "BranchCrossEventsExportError",
...
func omniCtx(arg any, client *sdk.BranchCrossEventsExportSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BranchCrossEventsExportSDK); is {
			client = live
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.BranchCrossEventsExportError{Msg: msg}
			}
		}
	}
...
func omniErrArg(arg any) error {
	...
	return &sdk.BranchCrossEventsExportError{Msg: msg, Code: code}
}
