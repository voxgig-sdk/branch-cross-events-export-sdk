...
class BranchCrossEventsExportMakeError
{
    public static function call(?BranchCrossEventsExportContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BranchCrossEventsExportContext([], null);
        }
        $op = $ctx->op ?? new BranchCrossEventsExportOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BranchCrossEventsExportResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BranchCrossEventsExportError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BranchCrossEventsExportError('', $msg, $ctx);
        ...
        if ($err instanceof BranchCrossEventsExportError) {
