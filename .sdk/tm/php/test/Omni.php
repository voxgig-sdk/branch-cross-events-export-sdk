...
//    only - a typed BranchCrossEventsExportContext there would make every ctx
...
final class BranchCrossEventsExportOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BranchCrossEventsExportContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new BranchCrossEventsExportSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new BranchCrossEventsExportResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new BranchCrossEventsExportError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new BranchCrossEventsExportResponse($clean['response']);
    ...
    public static function err_from_map($m): ?BranchCrossEventsExportError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BranchCrossEventsExportError(is_string($code) ? $code : '', $msg);
    }
