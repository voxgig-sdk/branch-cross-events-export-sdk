...
// BranchCrossEventsExport SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class BranchCrossEventsExportTestFeature extends BranchCrossEventsExportBaseFeature
{
    ...
    public function init(BranchCrossEventsExportContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (BranchCrossEventsExportContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = BranchCrossEventsExportParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (BranchCrossEventsExportContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(BranchCrossEventsExportContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = BranchCrossEventsExportParam::call($ctx, $k);
