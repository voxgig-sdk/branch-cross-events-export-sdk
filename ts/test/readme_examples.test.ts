...
const SDK_NAME = 'BranchCrossEventsExportSDK'
...
const TEST_SEED = {"entity":{"analytics":{"test01":{"id":"test01"}},"status":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+BranchCrossEventsExportSDK\b/.test(code) ||
    /\bBranchCrossEventsExportSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BranchCrossEventsExportSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BranchCrossEventsExportSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
