# BranchCrossEventsExport TypeScript SDK



The TypeScript SDK for the BranchCrossEventsExport API — a type-safe, entity-oriented client with full async/await support.
...
```ts
import { BranchCrossEventsExportSDK } from '@voxgig-sdk/branch-cross-events-export'

const client = new BranchCrossEventsExportSDK()
```
...
```ts
try {
  const status = await client.Status().load({ id: "example_id", app_id: "example" })
  console.log(status)
} catch (err) {
...
```ts
const client = BranchCrossEventsExportSDK.test()

const status = await client.Status().load({ id: 'test01', app_id: 'example_app_id' })
// status is the entity, populated with mock response data
// — call status.data() for the record itself
console.log(status)
```
...
```ts
const entity = client.Status()

// First call runs the operation and stores its result
await entity.load({ id: 'example', app_id: 'example_app_id' })
...
```ts
new BranchCrossEventsExportSDK(options?: {
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `BranchCrossEventsExportSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `BranchCrossEventsExportSDK.test(testopts?, sdkopts?)` | `BranchCrossEventsExportSDK` | Create a test-mode client. |
...
| `client` | `client(): BranchCrossEventsExportSDK` | Return the parent SDK client. |
...
| `filter` | `Record<string, any>` | An object representing a filter, or multiple filters. |
| `granularity` | `string` | The time granularity that each band value will represent. |
| `job_id` | `string` | The unique ID of the request. |
| `limit_sort_spec` | `Record<string, any>` | An object describing column/row ordering and limiting. |
...
```ts
const status = await client.Status().load({ id: 'status_id', app_id: 'app_id' })
```
...
```
branch-cross-events-export/
├── src/
│   ├── BranchCrossEventsExportSDK.ts        # Main SDK class
...
await status.load({ id: "example_id", app_id: "example" })

// status.data() now returns the status data from the last `load`
// status.match() returns { id: "example_id" }
```
