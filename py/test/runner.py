# BranchCrossEventsExport SDK test runner

from __future__ import annotations
import os
import json

from branchcrosseventsexport_sdk.utility.voxgig_struct import voxgig_struct as vs


class BranchCrossEventsExportTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BranchCrossEventsExportTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BranchCrossEventsExportTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BranchCrossEventsExportTestRunner.getenv("BRANCH_CROSS_EVENTS_EXPORT_TEST_LIVE")
        override = BranchCrossEventsExportTestRunner.getenv("BRANCH_CROSS_EVENTS_EXPORT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BranchCrossEventsExportTestRunner.getenv(key)
        ...
        explain = BranchCrossEventsExportTestRunner.getenv("BRANCH_CROSS_EVENTS_EXPORT_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BRANCH_CROSS_EVENTS_EXPORT_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BranchCrossEventsExportTestRunner._test_control is not None:
            return BranchCrossEventsExportTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BranchCrossEventsExportTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BranchCrossEventsExportTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BranchCrossEventsExportTestRunner.load_test_control()
...
def load_env_local():
    BranchCrossEventsExportTestRunner.load_env_local()


def env_override(m):
    return BranchCrossEventsExportTestRunner.env_override(m)


def entity_data(v):
    return BranchCrossEventsExportTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BranchCrossEventsExportTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BranchCrossEventsExportTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BranchCrossEventsExportTestRunner.load_test_control()


def live_client_options():
    return BranchCrossEventsExportTestRunner.live_client_options()


def live_delay_ms():
    return BranchCrossEventsExportTestRunner.live_delay_ms()
