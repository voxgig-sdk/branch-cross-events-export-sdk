# BranchCrossEventsExport Python SDK



The Python SDK for the BranchCrossEventsExport API — an entity-oriented client following Pythonic conventions.
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/branch-cross-events-export-sdk/releases)) or
...
client = BranchCrossEventsExportSDK()
```
...
# status contains the mock response record
```
...
client = BranchCrossEventsExportSDK(options)
```
...
| --- | --- | --- |
| `aggregations` | `list` | An array where each element is an aggregation object. |
| `code` | `int` |  |
| `dimensions` | `list` | An array representing dimension(s) to group by. |
| `end_date` | `str` | The end of the interval time range represented as an ISO-8601 complete date. |
| `filter` | `dict` | An object representing a filter, or multiple filters. |
| `granularity` | `str` | The time granularity that each band value will represent. |
| `job_id` | `str` | The unique ID of the request. |
| `limit_sort_spec` | `dict` | An object describing column/row ordering and limiting. |
| `response_format` | `str` | Compression format to use for data file. |
| `response_format_compression` | `str` | Compression method to use for data file. |
| `start_date` | `str` | The start of the interval time range represented as an ISO-8601 complete date. |
...
```
py/
├── branchcrosseventsexport_sdk.py         -- Main SDK module
...
The main module (`branchcrosseventsexport_sdk`) exports the SDK class.
