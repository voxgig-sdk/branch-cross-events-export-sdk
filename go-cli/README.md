# branch-cross-events-export-cli

boru-driven command-line client **and** interactive REPL for the BranchCrossEventsExport
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-cross-events-export-cli)
make build

# 2. See usage (words, entities, env vars)
./branch-cross-events-export-cli --help
...
BRANCH_CROSS_EVENTS_EXPORT_BASE=https://api.example.com ./branch-cross-events-export-cli --help

# 6. No arguments -> interactive REPL
./branch-cross-events-export-cli
branch-cross-events-export> /help
branch-cross-events-export> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-cross-events-export-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/branch-cross-events-export-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `branch-cross-events-export>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/branch-cross-events-export-cli`. |
...
(`./branch-cross-events-export-cli <expr>`) and interactively (the REPL), and expressions compose the
