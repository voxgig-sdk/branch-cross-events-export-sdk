// Command BranchCrossEventsExport-cli is a CLI + REPL for the BranchCrossEventsExport SDK,
...
const prompt = "branch-cross-events-export"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "analytics status"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BRANCH_CROSS_EVENTS_EXPORT_APIKEY (api key), BRANCH_CROSS_EVENTS_EXPORT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BRANCH_CROSS_EVENTS_EXPORT_APIKEY carries the API key and
	// BRANCH_CROSS_EVENTS_EXPORT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BRANCH_CROSS_EVENTS_EXPORT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BRANCH_CROSS_EVENTS_EXPORT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBranchCrossEventsExportSDK(opts)
