# BranchCrossEventsExport JavaScript SDK



The JavaScript SDK for the BranchCrossEventsExport API — an entity-oriented client with full async/await support.
...
## Install
```js
npm install branch-cross-events-export
```
...
```js
const { BranchCrossEventsExportSDK } = require('@voxgig-sdk/branch-cross-events-export-js')
...
| `filter` | `Object` | An object representing a filter, or multiple filters. |
| `granularity` | `string` | The time granularity that each band value will represent. |
| `job_id` | `string` | The unique ID of the request. |
| `limit_sort_spec` | `Object` | An object describing column/row ordering and limiting. |
...
├── src/
│   ├── BranchCrossEventsExportSDK.js        # Main SDK class
