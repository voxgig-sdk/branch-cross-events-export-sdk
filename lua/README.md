# BranchCrossEventsExport Lua SDK



The Lua SDK for the BranchCrossEventsExport API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Analytics()` — each with the same small set of operations (`load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/branch-cross-events-export-sdk/releases)),
...
```lua
-- Create
local created, err = client:Analytics():create({ end_date = "example_end_date", start_date = "example_start_date" })
...
```lua
local status, err = client:Status():load({ id = "example_id", app_id = "example" })
if err then error(err) end
```
...
local result, err = client:Status():load({ id = "test01", app_id = "example" })
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local status, err = client:Status():load({ id = "example_id" })
    if err then error(err) end
    -- status is the loaded record
...
| --- | --- | --- |
| `aggregations` | `table` | An array where each element is an aggregation object. |
| `code` | `number` |  |
| `dimensions` | `table` | An array representing dimension(s) to group by. |
| `end_date` | `string` | The end of the interval time range represented as an ISO-8601 complete date. |
| `filter` | `table` | An object representing a filter, or multiple filters. |
| `granularity` | `string` | The time granularity that each band value will represent. |
| `job_id` | `string` | The unique ID of the request. |
| `limit_sort_spec` | `table` | An object describing column/row ordering and limiting. |
...
```lua
local status, err = client:Status():load({ id = "status_id", app_id = "app_id" })
```
...
```
lua/
├── branch-cross-events-export_sdk.lua    -- Main SDK module
...
The main module (`branch-cross-events-export_sdk`) exports the SDK constructor
