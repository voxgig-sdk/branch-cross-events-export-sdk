# BranchCrossEventsExport Lua SDK Reference

Complete API reference for the BranchCrossEventsExport Lua SDK.
...
```lua
local sdk = require("branch-cross-events-export_sdk")
...
| --- | --- | --- | --- |
| `aggregations` | `table` | No | An array where each element is an aggregation object. |
| `code` | `number` | No |  |
| `dimensions` | `table` | No | An array representing dimension(s) to group by. |
| `end_date` | `string` | Yes | The end of the interval time range represented as an ISO-8601 complete date. |
| `filter` | `table` | No | An object representing a filter, or multiple filters. |
...
| `limit_sort_spec` | `table` | No | An object describing column/row ordering and limiting. |
...
```lua
local result, err = client:Status():load({ id = "status_id", app_id = "app_id" })
```
