...
require("branch-cross-events-export_types")
...
local BranchCrossEventsExportSDK = {}
BranchCrossEventsExportSDK.__index = BranchCrossEventsExportSDK
...
BranchCrossEventsExportSDK._make_feature = _make_feature


function BranchCrossEventsExportSDK.new(options)
  local self = setmetatable({}, BranchCrossEventsExportSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BranchCrossEventsExportSDK:options_map()
...
function BranchCrossEventsExportSDK:get_utility()
  return Utility.copy(self._utility)
end


function BranchCrossEventsExportSDK:get_root_ctx()
  return self._rootctx
end


function BranchCrossEventsExportSDK:prepare(fetchargs)
...
function BranchCrossEventsExportSDK:direct(fetchargs)
...
function BranchCrossEventsExportSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function BranchCrossEventsExportSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "BranchCrossEventsExportSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function BranchCrossEventsExportSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BranchCrossEventsExportSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BranchCrossEventsExportSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BranchCrossEventsExportSDK:Analytics(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BranchCrossEventsExportSDK:Status(data)
...
function BranchCrossEventsExportSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BranchCrossEventsExportSDK.new(sdkopts)
  sdk.mode = "test"
...
return BranchCrossEventsExportSDK
