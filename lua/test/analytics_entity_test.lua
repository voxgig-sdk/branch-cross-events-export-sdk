...
describe("AnalyticsEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set BRANCH_CROSS_EVENTS_EXPORT_TEST_ANALYTICS_ENTID JSON to run live")
      return
    end
    ...
    local analytics_ref01_data = helpers.to_map(vs.getprop(
...
function analytics_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("BRANCH_CROSS_EVENTS_EXPORT_TEST_ANALYTICS_ENTID")
  ...
  if env["BRANCH_CROSS_EVENTS_EXPORT_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["BRANCH_CROSS_EVENTS_EXPORT_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
